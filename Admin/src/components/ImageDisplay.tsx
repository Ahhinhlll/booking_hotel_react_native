...
interface ImageDisplayProps {
  ...
  height?: number;
  preview?: boolean;
  showCount?: boolean; // Hiển thị số lượng ảnh
}
...
const ImageDisplay = ({ 
  ...
  if (!images || images.length === 0) {
    return <span style={{ color: '#999' }}>Chưa có ảnh</span>;
  }

  // Chỉ hiển thị ảnh đầu tiên, nhưng cho phép preview tất cả
  ...
  return (
    ...
    >
      <Badge 
        count={showCount && imageCount > 1 ? `+${imageCount - 1}` : 0}
        style={{ backgroundColor: '#1890ff' }}
      >
        <Image
          width={width}
          height={height}
          src={uploadService.getImageUrl(firstImage)}
          alt="Image"
          style={{ 
            objectFit: 'cover', 
            borderRadius: 4,
