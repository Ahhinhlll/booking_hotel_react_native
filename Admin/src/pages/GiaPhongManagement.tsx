...
const GiaPhongManagement = () => {
  ...
  const handleSearch = async () => {
    ...
    setLoading(true);
    try {
      const data = await giaPhongService.search(searchText);
      setGiaPhongs(Array.isArray(data) ? data : []);
    } catch (error) {
      console.error('Error searching room prices:', error);
  ...
  const handleSubmit = async (values: any) => {
    try {
      if (editingGiaPhong) {
        await giaPhongService.update({ ...values, maGiaPhong: editingGiaPhong.maGiaPhong });
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (loaiDat: string) => (
        <Tag color={loaiDat === 'Theo giờ' ? 'blue' : loaiDat === 'Theo ngày' ? 'green' : 'orange'}>
          {loaiDat}
        </Tag>
      ),
    },
    ...
    <div>
      ...
      >
        <Form form={form} layout="vertical" onFinish={handleSubmit}>
          ...
          >
            <Select showSearch optionFilterProp="children" placeholder="Chọn phòng">
              {rooms.map((room) => (
          ...
          <Form.Item
            ...
            name="gia2GioDau"
            rules={[{ required: true, message: 'Vui lòng nhập giá 2 giờ đầu!' }]}
          >
            <InputNumber
              style={{ width: '100%' }}
          ...
          <Form.Item
            ...
            name="gia1GioThem"
            rules={[{ required: true, message: 'Vui lòng nhập giá 1 giờ thêm!' }]}
          >
            <InputNumber
              style={{ width: '100%' }}
              formatter={(value) => `${value}`.replace(/\B(?=(\d{3})+(?!\d))/g, ',')}
